...
Usage:
  ...
  python scripts/collect_data.py --shards 375 --sims 50 --outdir data/all_data_v4 --start-shard 0
  python scripts/collect_data.py --shards 375 --sims 50 --outdir data/all_data_v4 --start-shard 375
...
def main():
    ...
    parser.add_argument('--sims',        type=int, default=COLLECTION_SIMS)
    parser.add_argument('--outdir',      type=str,
                        default=os.path.join(DATA_DIR, 'ismcts_experiences'))
    ...
    if args.start_shard is None and not args.resume and existing:
    ...
    spg   = t1 - t0
    epg   = len(exps)
    games = (args.shards * EXP_PER_SHARD) // max(epg, 1)
    ...
    while shard_num < start_shard + args.shards:
        hands = deal(rng)
        exps  = collect_game(
            bots, hands, seed=42 + start_shard + game_count
        )
        buffer.extend(exps)
        game_count += 1
